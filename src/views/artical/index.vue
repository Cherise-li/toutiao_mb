...
import { getArticalDetail, cancelLike, addLike, cancelLove, disLove } from '@/api/artical'
export default {
  name: 'ArticleIndex',
  data () {
    return {
  ...
  methods: {
    async hLove () {
      if (this.article.attitude === 0) {
        // 取消不喜欢
        await cancelLove(this.$route.params.id)
        this.article.attitude = 1
      } else {
        // 不喜欢
        await disLove(this.$route.params.id)
        this.article.attitude = 0
      }
    },
    async getArtical () {
      this.loading = true
      const res = await getArticalDetail(this.$route.params.id)
